...
def add(bazel_content: str, session: Session) -> int:
    ...
    """
    logger.debug(f"Adding bazel: {bazel_content}...")
    ...
    try:
        ...
        if exists(content_hash, session=session):
            logger.debug("Bazel is already present in db!")
            return 0
        # Create bazel object
        ...
        logger.debug("Bazel added to db")
        return 1
...
def list(session: Session) -> list[Bazel]:
    ...
    """
    logger.debug("Retrieving all bazels...")
    try:
        # Add bazel object
        bazels = session.query(Bazel).all()

        logger.debug(f"{len(bazels)} Bazels successfully retrieved!")

        return bazels
    except Exception as exc:
        logger.error(f"Bazels could not be retrieved: {exc}")
        raise exc
...
def get(bazel_content: str, session: Session) -> Bazel:
    ...
    Args:
        ...
        session (Session): The session.
    """
    logger.debug(f"Retrieving bazel: {bazel_content}...")
    try:
        ...
        logger.debug(f"Bazel with ID: { bazel.id } successfully retrieved!")

        return bazel
    except Exception as exc:
        logger.debug(f"Bazel could not be retrieved: {exc}")
        raise exc
...
def delete(bazel_content: str, session: Session):
    ...
    Args:
        ...
        session (Session): The session.
    """
    logger.debug(f"Deleting bazel: {bazel_content}...")
    try:
        # Add bazel object
        bazels = session.query(Bazel).filter(Bazel.content == bazel_content).delete()

        logger.debug(f"{len(bazels)} Bazel successfully deleted!")
    except Exception as exc:
        logger.debug(f"Bazel could not be deleted: {exc}")
...
def count(session: Session = get_session()) -> int:
    ...
    """
    logger.debug("Counting all bazels...")
    try:
        # Add bazel object
        bazel_count = session.query(func.count(Bazel.id)).scalar()

        logger.debug(f"There are {bazel_count} bazels in the db!")

        return bazel_count
    except Exception as exc:
        logger.error(f"Bazels could not be counted: {exc}")
        raise exc
